...
- **break_corrections**: 休憩修正申請

各テーブルの詳細はプロジェクト内の `knowledge.md` を参照してください。
...
- **命名規則**:
  ...
  - クラス: PascalCase
  - 定数: SNAKE_CASE
- **型宣言**: 全メソッドに型宣言（引数・戻り値）
- **N+1対策**: Eager Loading（`with()` / `load()`）を必須

### パフォーマンス最適化
- マジックナンバーは定数化
- デッドコードは削除
- CollectionメソッドのMap/filter等を活用
- `foreach` より高階関数を優先

### CSS規約
- IDセレクタ禁止
- `!important` 禁止
- クラス名はハイフン区切り（`.btn-primary`）

---

## 📖 参考資料

- [Laravel 8.x ドキュメント](https://laravel.com/docs/8.x)
...
プロジェクト管理・メンテナンスに関するお問い合わせは管理者までお願いします。
